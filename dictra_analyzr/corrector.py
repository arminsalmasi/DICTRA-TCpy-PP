...
class ResultCorrector:
            ...
            for phase in phases_to_process:
                ...
                for pt in range(n_pts):
                    ...
                    self._process_phase_points(
                        pt, phase, phXs, elnames, nelements, n_pts,
    ...
    def _process_phase_points(self, pt, phase, phXs, elnames, nelements, n_pts,
                              search_element_idx, cutoff, phase_to_change,
                              CQT_tS_TC_NEAT_phXs, CQT_tS_TC_NEAT_npms):
        if not np.any(phXs > 0):
            return

        sorted_indices = np.flip(np.argsort(phXs))
        search_idx = np.where(sorted_indices == search_element_idx)[0][0]

        if self._check_condition_met(phXs, search_element_idx, cutoff, search_idx):
    ...
    def _get_new_phase_name(self, phase_to_change, sorted_elnames, cutoff):
        if cutoff == 1:
             return phase_to_change + '-' + sorted_elnames[0]
        elif cutoff > 0 and cutoff < 1:
             return phase_to_change + '-' + "".join(sorted_elnames[:2])
        else:
             return phase_to_change + '-' + "".join(sorted_elnames)
